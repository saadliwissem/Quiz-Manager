...
router.post('/quiz', upload.single('image'), async (req, res) => {
    try {
      ...
      const quiz = new Quiz({ title, description, image });
      await quiz.save();
...
// Update a quiz
router.put('/quiz/:id', upload.single('image'), async (req, res) => {
    try {
      const { id } = req.params;
      const { title, description } = req.body;
      const image = req.file ? req.file.path : null; // Save image path if uploaded
      const quiz = await Quiz.findByIdAndUpdate(id, { title, description, image }, { new: true });
      if (!quiz) {
